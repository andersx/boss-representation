...
if __name__ == "__main__":
    ...
    for i, rep1 in enumerate(X): 
        for j, rep2 in enumerate(X):
            # L[i,j] = kernel(rep1, rep2, sigma)
            K[i,j] = fboss.boss_kernel(training[i].coordinates, training[i].nuclear_charges, 
                                       training[j].coordinates, training[j].nuclear_charges)
        print i, K[i,:]
    ...
    for i, rep1 in enumerate(X): 
        for j, rep2 in enumerate(Xs):
            ...
            Ks[i,j] = fboss.boss_kernel(training[i].coordinates, training[i].nuclear_charges, 
                                       test[j].coordinates, test[j].nuclear_charges)
        print i, Ks[i,:]
